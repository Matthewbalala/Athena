...
import pdb
# import sys
# sys.path.append('/root/code/aigis/maddpg/maddpg/experiments')
from  aigisenv import AigisEnv
...
def get_trainers_lc(env, obs_shape_n, arglist):
    trainers = []
    model = mlp_model
...
def train_lc(arglist):
    ...
    with U.single_threaded_session():
        ...
        env = AigisEnv(booted=True, act_importance=53)
        ...
        while True:
            print("Train_step now: {}".format(train_step))
            # get action
            if ACT_INIT <= ACT_MIN:
                ACT_INIT = ACT_MIN
            else:
